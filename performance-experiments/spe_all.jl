...
b = M * b
b = b / norm(b)
x = testSddm(tests, dicWarmup, M, b; verbose=false, tol=1e-8, testName=tn, test_petsc_hypre=true,
...
for spe_size in ["0.5m", "2m", "4m", "8m", "16m"]
    ...
    b = M * b
    b = b ./ norm(b)
    x = testSddm(tests, dic, M, b; verbose=false, tol=5e-9, testName=tn, test_petsc_hypre=true,
    test_hypre=true, test_icc=true, test_cmg=true, test_lamg=true)
