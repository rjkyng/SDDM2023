...
b = b / norm(b)
dicWarmup = Dict()
x = testSddm(tests_sddm, dicWarmup, M, b; verbose=true, tol=1e-8, testName=tn, test_petsc_hypre=true,
...
x = testLap(tests_lap, dicWarmup, a, b; verbose=true, tol=1e-8, testName=tn, test_petsc_hypre=true,
test_hypre=true, test_icc=true, test_cmg=true, test_lamg=true)
@save fn dicWarmup  
...
for matname in _names 
    ...
    a, d = adj(M)
    if !isLap(M)
        x = testSddm(tests_sddm, dic, M, b; verbose=false, tol=1e-8, testName=tn, test_petsc_hypre=true,
        test_hypre=true, test_icc=true, test_cmg=true, test_lamg=true)
    else 
        x = testLap(tests_lap, dic, a, b; verbose=false, tol=1e-8, testName=tn, test_petsc_hypre=true,
        test_hypre=true, test_icc=true, test_cmg=true, test_lamg=true)
    end
